...
<Equation tex="R(\theta) = \sum_i \Pi_i \cdot \mathbb{E}_{(x,y)\sim D_i}\left[L(f_\theta(x), y)\right]" label="static ERM" />
...
<Equation tex="R^{(t)}(\theta) = \sum_i p_i^{(t)} \cdot \mathbb{E}_{(x,y)\sim D_i}\left[L(f_\theta(x), y)\right]" label="dynamic ERM" />
...
<Equation tex="p_i = c \cdot \Pi_i + (1 - c) \cdot w_i" label="blend" />
...
<Equation tex="R = \left\lfloor E / \mathrm{bf} \right\rfloor" />

Total runtime is:

<Equation tex="T_{\mathrm{ART}} = T_{\mathrm{baseline}} + \left\lfloor E / \mathrm{bf} \right\rfloor \cdot T_{\mathrm{refresh}}" />

Each refresh consists of:

<Equation tex="T_{\mathrm{refresh}} = T_{\text{val\_fwd}} + T_{\mathrm{metric}} + T_{\mathrm{resample}}" />
